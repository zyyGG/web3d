...
// const run_path = await gltfLoader.loadAsync('/models/blender_path.glb')
// run_path.scene.traverse((child) => {
//   console.log(child)
// })
const pyxLoader = new PLYLoader();
const run_path = await pyxLoader.loadAsync('/models/blender_path.ply')
console.log(run_path)
run_path.computeVertexNormals(); // 计算一下顶点
const material = new THREE.MeshStandardMaterial( { color: 0x009cff, flatShading: true } );
const mesh = new THREE.Mesh( run_path, material );
scene.add(mesh);
