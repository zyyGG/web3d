...
function createCircularRing(position, outRadius, inRadius, color) {
  ...
  geometry.setAttribute( 'position', new THREE.BufferAttribute( ringCircle(1, 2, 4), 3 ) );
...
// 竖着连接的矩形
// segment越大，越平滑
function ringCircle(inRadius, outRadius, segment = 4){
  const points = []
  const angle = 360 / segment // 每个点的角度
  // 按照segment分割内外圆环的点位
  for(let i = 0; i < segment; i++){
    ...
    points.push(point1) // 外圆环的点
    points.push(point4) // 内圆环的点
    points.push(point2) // 外圆环的点

    points.push(point1) // 外圆环的点
    points.push(point2) // 外圆环的点
    points.push(point3) // 内圆环的点
  }
