...
const playerHead = new BABYLON.MeshBuilder.CreateSphere("playerHead", { diameter: 0.5 }, scene);
playerHead.parent = player;
playerHead.position.set(0, 1.0, 0);
// 添加阴影投射者
shadowGenerator.addShadowCaster(playerHead, true) // 添加阴影投射者
const playerBody = new BABYLON.MeshBuilder.CreateCylinder("playerBody", { diameterTop: 0.3, diameterBottom: 0.3, height: 1 }, scene);
playerBody.parent = player;
playerBody.position.set(0, 0.5, 0);
// 添加阴影投射者
shadowGenerator.addShadowCaster(playerBody, true) // 添加阴影投射者

// 创建shape
const playerShape = new BABYLON.PhysicsShapeContainer(scene)
const headShape = new BABYLON.PhysicsShapeSphere(
  new BABYLON.Vector3(0, 1.0, 0), // 中心位置
  0.26, // 半径
  scene,
);
const bodyShape = new BABYLON.PhysicsShapeCylinder(
  new BABYLON.Vector3(0, 0, 0), // 点A
  new BABYLON.Vector3(0, 1, 0), // 点B
  0.2, // 底部半径
  scene,
);
playerShape.addChild(headShape, )
playerShape.addChild(bodyShape)
const playerRigidBody = new BABYLON.PhysicsBody(
  player,
  BABYLON.PhysicsMotionType.DYNAMIC, // 动态
  false, // 初次启动的是否启用
  scene,
);
playerRigidBody.shape = playerShape; // 设置形状
playerRigidBody.setMassProperties(1) // 设置质量
