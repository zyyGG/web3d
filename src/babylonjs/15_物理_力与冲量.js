...
const config = {
  ...
  forcePower: function () {
    ...
    for (const mesh of scene.rootNodes) {
      if(mesh.physicsBody) {
        /**@type {BABYLON.PhysicsBody} */
        const body = mesh.physicsBody
        // 第一个是力的向量, 第二个力的位置
        body.applyForce(new BABYLON.Vector3(0, 10, 0), new BABYLON.Vector3(0, 0, 0));
      }
    }
  },
  impulsePower: function (){
    // 找到场景中的所有刚体
    for (const mesh of scene.rootNodes) {
      if(mesh.physicsBody) {
        /**@type {BABYLON.PhysicsBody} */
        const body = mesh.physicsBody
        // 第一个是冲量的向量, 第二个冲量的位置
        body.applyImpulse(new BABYLON.Vector3(0, 10, 0), new BABYLON.Vector3(0, 0, 0));
      }
    }
  }
}
...
physicsGui.add(config, "physicsShow")
  .name("刚体边框")
  .onChange((value) => {
    if(value == true) {
      // 清理旧的物理体
...
physicsGui.add(config, "forcePower").name("施加力")
physicsGui.add(config, "impulsePower").name("施加冲量")

physicsGui.open();

/**
 * 力与冲量
 * 1. applyForce 施加力，力是持续的， 比如重力，阻力，玩家推动物体的力
 * 2. applyImpulse 施加冲量，这个是瞬间的力，例如撞击时的反作用力
 */
...
for(let i = 0; i < colSize; i ++){
  for(let j = 0; j < colSize; j++){
    for(let k = 0; k < colSize; k++){
      const position = {
        x: i + i * padding - colSize / 2,
        y: j + j * padding ,
        z: k + k * padding - colSize / 2,
      }
      positions.push(position)
      const cube = createRigiCube(position)
    }
  }
}

function createRigiCube(position){
  /**@type {BABYLON.Mesh} */
  const cube = new BABYLON.MeshBuilder.CreateBox("cube", {size: 1}, scene)
  ...
  cubeRigidBody.shape = cubeShape
  cubeRigidBody.setMassProperties({
    mass: 1,
  })
  return cube
}
